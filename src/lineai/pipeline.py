...
_SALES_HINT_RE = re.compile(
    r"買|購|下單|訂|價|錢|多少|報價|庫存|有貨|現貨|缺貨|調貨|到貨|"
    r"便宜|優惠|折|划算|推薦|規格|幾元|幾塊|單價|售價|成本|出貨"
)
...
def classify_intent(user_text: str) -> str:
    """判斷 L1 該走哪條路：'sales'（要查資料庫）或 'chat'（只用長期記憶）。

    判定為銷售意圖（需要查貨況/價格）的條件：
      - 句中含有商品型號（如 RTX5060），或
      - 句中含有銷售相關字眼（買、價格、多少錢、庫存、推薦…）。
    其餘（純打招呼、客服詢問、閒聊）一律走 chat，平時不碰資料庫。
    """
    text = (user_text or "").strip()
    if not text:
        return "chat"
